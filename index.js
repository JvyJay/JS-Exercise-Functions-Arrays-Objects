...
function getLastCarInfo(inventory) {
  const Info1st = inventory.find((item, index) => {
    return index === 49
  })
  return `This is a ${Info1st.car_make} ${Info1st.car_model}`
}
...
function getCarInfoById(inventory, id) {
  const Info1st = inventory.find((item, id) => {
    return id === 0
  })
  return `This is a ${Info1st.car_make} ${Info1st.car_model}`
}
...
function sortCarInventory(inventory) {
  function sorting(car_model) {
    var sortOrder = 1;

    if (car_model[0] === "-") {
      sortOrder = -1;
      car_model = car_model.substr(1);
    }

    return function (a, b) {
      if (sortOrder == -1) {
        return b[car_model].localeCompare(a[car_model]);
      } else {
        return a[car_model].localeCompare(b[car_model]);
      }
    }
  }
  const carSort = inventory.sort(sorting("car_model"));
  return carSort;
}
...
function getModelYears(inventory) {
  // const test = inventory.map(function (car) {
  //   return car.car_year;
  // })
  // return test;
  const test = [];
  for (let i = 0; i < inventory.length; i++) {
    test.push(inventory[i].car_year);
  }
  return test;
}
